...
const Matches = () => {
  ...
  const handleReject = async (userId) => {
    try {
      ...
      setPendingMatches(pendingMatches.filter(match => match.userId !== userId));
    } catch (err) {
  ...
  const renderMatchCard = (match, isConnection = false, isPending = false) => (
...
const fetchPotentialMatches = async () => {
  try {
    setLoading(true);
    setError('');
    const response = await matchAPI.getPotentialMatches();
...
const fetchConnections = async () => {
  try {
    setLoading(true);
    setError('');
    const response = await matchAPI.getAcceptedMatches();
    if (response.data) {
      setConnections(response.data.connections || []);
      setPendingMatches(response.data.pendingMatches || []);
    }
  } catch (err) {
    const errorMessage = err.response?.data?.error || 'Failed to fetch connections. Please try again.';
    setError(errorMessage);
    showSnackbar(errorMessage, 'error');
  } finally {
    setLoading(false);
...
const handleAccept = async (userId) => {
  try {
    const response = await matchAPI.acceptMatch(userId);
    showSnackbar(
      response.data?.isMutual 
...
const renderMatchCard = (match, isConnection = false, isPending = false) => (
  <Card key={match.userId} sx={{ mb: 3, maxWidth: 600, mx: 'auto' }}>
    <CardContent>
      ...
      <Box mb={2}>
        {match.commonSubjects?.teach?.length > 0 && (
          <>
        ...
        {match.commonSubjects?.learn?.length > 0 && (
          <>
    ...
    <CardActions sx={{ justifyContent: 'flex-end', p: 2, pt: 0 }}>
      ...
      {tabValue === 2 && !isConnection && (
        ...
        >
          {match.status === 'accepted' ? 'Request Sent' : 'Accept'}
        </Button>
      )}
      {isConnection && (
        <Button 
          ...
          variant="outlined" 
          color="primary"
          // Add onClick handler for messaging
        >
          Message
        </Button>
            ...
            <Box mb={2}>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mt: 0.5 }}>
                {user.subjectsToLearn.map((subject, index) => (
                  <Chip key={index} label={subject} size="small" color="secondary" />
                ))}
              </Box>
            </Box>
          )}
          ...
          <TabPanel value={tabValue} index={2}>
            ...
            {pendingMatches.map(match => (
              <Grid item xs={12} key={match.userId}>
                {renderMatchCard(match, false, true)}
              </Grid>
            ))}
          </Grid>
        )}
